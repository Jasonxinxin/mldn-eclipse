...
<%!
	public static final String SUPPLY_EMP_APPLY_HANDLE_URL = "pages/back/admin/finance/finance_reimbursement_audit.action" ;
%>
			...
			<div class="panel-body">
				<div id="reimbursementInfoDiv" class="row">
					<div class="col-xs-12">
						...
						<input type="hidden" id="rbsid" name="spid" value="${rbsid}">
							<tr>
							...
							<tr>
								...
								<td><span id="cost">${reimbursement.money}</span></td>
							</tr>
							<tr>
								<td><strong>报销单说明：</strong></td>
								<td><pre class="pre-scrollable" style="width:500px;height:100px;">${reimbursement.note}</pre></td>
							</tr>
						</table>
					</div>
				</div>
				<div id="detailsTable" class="row">
					<table class="table table-condensed">
						...
						<tbody>
						<c:forEach items="${reimbursementDetailList}" var="reimbursementDetail">
							<tr>
								<td class="text-center">${reimbursementDetail.title}</td>
								<td class="text-center">${reimbursementDetail.money}</td>
				...
				<div>
					<form class="form-horizontal" action="<%=SUPPLY_EMP_APPLY_HANDLE_URL%>" id="myform" method="post">
						<input type="hidden" id="rbsid" name="rbsid" value="${rbsid}">
						<fieldset>
							...
							<div class="form-group" id="noteDiv">
								...
								<div class="col-md-6">
									...
									<textarea id="note" name="note" rows="3"
										class="form-control" placeholder="请输入审核所给出的意见信息" rows="10">${reimbursement.anote}</textarea>
								</div>
								<!-- 定义表单错误提示显示元素 -->
